...
2. 1.21.11 客户端整合包
3. 重构镐和剑的抽奖
4. 开发服务器网站，制作登录系统与白名单验证系统
5. 玩家轨迹可视化

## 长期计划 {#prospect}

先暂时列出大饼，这些至少要到半年以后才能执行了。不一定保证完成，而且需要较长的周期，可以作为服务器未来发展的路线参考。

1. 玩家可以通过网站发表评论、建议或漏洞报告
2. 服主可以在网站内发布提案和专门的计划列表（而不是目前一个简陋的页面）
3. 可以通过网站查看玩家统计数据
4. 考虑对QQ或微信群机器人的支持
5. 增加空岛维度
6. 在B站提供新人入门指南视频。
7. 原QQ群的群主号被永久封禁，计划重建新群。并要求所有白名单玩家重新填写联系方式。
8. 让⭐币有用武之地
9. 反作弊系统
...
2. 新增服务器信息展示页面
3. 增加*无法进服排查方案*文档
4. 修复了新人无法进服的bug（来自[VMP](https://github.com/RelativityMC/VMP-fabric/issues/105)）
5. 更换了网络线路，具有更好的网络稳定性和流畅度
6. 将服务器模组的映射表从yarn换成了官方表
7. 修复了出生点和玩家去大厅的有关bug
8. 将服务端优化模组上传至 Modrinth
9. 更新了Minebbs和MCMOD上的宣传图
10. 增加了玩家轨迹记录的功能
11. 处于旁观模式的玩家可以使用tp命令
12. 服务器更新至 1.21.11
13. Ledger 不再记录一万格以外区域的内容
14. 重新启用了 Async 和 ModernFix 模组

## 0.3.0
2025-10-01

1. 服务器被 DDoS 攻击，增设了高防服务器
2. 服务器被尝试登录爆破，初步加强了TCP有关防御，发出大量TCP请求的用户会被封禁IP
3. 关闭了进服自动获取皮肤，需手动制定，防止后台日志被大量找不到皮肤的报错占据
4. Geyser 增加了 GeyserOptionalPack，以优化基岩版体验
5. 开启了 Carpet 的 TNT 爆炸优化
6. 禁用了 Async 模组
7. 设计了自动模组更新、Fabric 启动器更新工具，以及指定坐标自动删区块工具
8. 明文规定了服务器反作弊要求
9. 添加了 Inventory Sorter 模组，服务端也可自动整理背包或容器
10. 服务器更新至 1.21.10，删除了大量空闲区块，调整了不同维度的服务器边界
~~11. 修复了快捷潜影盒复制物品的bug~~
11. 移除了快捷潜影盒
12. 移除了鞘翅配方，增加了鞘翅的抽奖概率
13. 假人下线后会显示操作者
14. 首次进服时显示*服务器行为准则*
15. 恢复了某些误删的区块
16. 位移了一下整合包文档
...
7. Java 更新至 25
8. 尝试修复随机的JVM崩溃问题，包括多次更换系统，失败
9. 卫星地图采用了 BlueMap
10. 建立起服务器独立的 Git 代码仓库
11. 使用 Fail2Ban 防御 SSH 爆破。
12. 服务器卡顿时自动打印主线程堆栈以便分析。
